...
This utilizes the [Faction](#polityfaction) class which is the main way GameObjects can communicate with Polity Manager. You can utilize this class to build your own classes with this as a field.
...
Polity Manager is suited for games that need to manage various groups of NPCs, especially when these relationships are a bit more complex, such as when one NPC needs to react to an enemy of one or more allied NPCs. However, it can also be applicable to simple teams.

It is also designed to be very bare bones so that you can modify it more easily to suit your game's specific needs. It's less of an all-encompassing solution and more of a framework.
...
All classes in this package are under the `Polity` namespace.

### Polity.Manager

All public methods can be called from this PolityManager Singleton, referenced as `PM`, for example `PM.ChangeRelation();`

#### ChangeRelation()

Sets a new relationship status between two factions based on their names or indices, adjusting their relation to either Neutral, Allies, or Enemies.
If the factions match, the `OnRelationChange` event will be invoked to notify all subscribers of the relation change.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `factionName`      | `string`         | The name of the faction initiating the relationship change. |
| `theirFactionName` | `string`         | The name of the faction that is targeted for the relationship change. |
| `factionRelation`  | `Polity.Relation` | The new relation to set; can be `Neutral`, `Allies`, or `Enemies`.|

#### CheckRelation()

Compares the relation of one faction to another and returns the enum that indicates their relation if found.
The overload methods accept `Member`, `IMember`, `Faction`, `string` names, or `int` indices.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `member`           | `Member`         | The member of the faction initiating the relationship comparison. |
| `theirMember`      | `Member`         | The member of the faction that is targeted for comparison. |
...
Serializes the `Polity.Relation[,]` matrix and sets the serialized `relationMatrixJSON` to its return value.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `RelationMatrix`     | `Polity.Relation[,]`   | The 2D Relation matrix. This can be omitted which will use the Singleton's `RelationMatrix`. |
...
Deserializes a string representing the `Polity.Relation[,]` matrix.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `json`     | `string`   | This can be omitted which will then use the Singleton's `relationMatrixJSON`. |
...
Adds a new faction to the manager and initializes its relationship entries in the matrix as `Neutral`.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `name`             | `string`         | The name of the new faction to add. |

#### RemoveFaction()

Removes a faction from the manager by name or index, updating the relation matrix accordingly.

| Parameter          | Type             | Description |
|--------------------|------------------|-------------|
| `name` or `index`  | `string` or `int` | The name or index of the faction to remove. |
...
Adds a member to the leader's members list.
...
Removes a member from the leader's members list.
